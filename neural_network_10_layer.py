...
def plot_graph(all_nodes, data_sample):
	...
	plt.legend()
	plt.title("10-layer ROC %s data" % data_sample)
	plt.savefig("./NN results/%s data/10-layer ROC %s" % (data_sample,data_sample))
	...
	plt.legend()
	plt.title("10-layer Probability Threshold %s data" % data_sample)
	plt.savefig("./NN results/%s data/10-layer Probability Threshold %s" % (data_sample,data_sample))
	...
	plt.savefig("./NN results/%s data/10-layer Filtered Jet Mass %s" % (data_sample,data_sample))
...
def structure_test():
	...
	for tries in range (int(sys.argv[2])):
		...
		print()
		print("Structure",tries,": ", n_nodes_hl1,n_nodes_hl2,n_nodes_hl3,n_nodes_hl4,n_nodes_hl5,n_nodes_hl6,n_nodes_hl7,n_nodes_hl8,n_nodes_hl9,n_nodes_hl10)
